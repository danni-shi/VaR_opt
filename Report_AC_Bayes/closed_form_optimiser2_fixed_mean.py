...
# To store results
delta_results = {}
gamma_list = [[1.25e-3, np.float64(0.0012), np.float64(0.00125)],   
 [1e-3, np.float64(0.0012), np.float64(0.001)],
 [7.5e-4, np.float64(0.0012), np.float64(0.00075)],
 [5e-4, np.float64(0.0012), np.float64(0.0005)],
 [0, np.float64(0.0012), np.float64(0.0)]]
...
for gamma in gamma_list:
    ...
    for run in range(num_runs):
        ...
        gamma_samples[gamma_samples > 0.005] = 0.00499
        # build model
        model = gp.Model("Closed_Form")
        model.setParam('OutputFlag', 1)
        model.setParam('Threads', 96)
        ...
        if model.status == GRB.OPTIMAL:
            ...
            print(f"No optimal solution found for prior of sample size {gamma[0]} on run {run}. Model Status = {model.status}")
        # --- END NEW ---
...
import pickle
results_dir = f"closed_form_prior_quick_fixed_mean"
os.makedirs(results_dir, exist_ok=True)
pickle_file = os.path.join(results_dir, "closed_form_prior_fixed_mean.pkl")
