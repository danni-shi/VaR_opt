...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--var_list',
        type=json.loads,
        default=[[1.41e-6, 27.93], [1.91e-6, 9.24]], 
        help='JSON string: List of trade sizes for the corresponding proxy distributions')
    ...
    for n_trade in var_list: #### <----------------------------------
        opt_timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        theta_v, alpha_v = n_trade[0], n_trade[1]
        print(f"Theta_Var = {theta_v}, Alpha_Var = {alpha_v}")
